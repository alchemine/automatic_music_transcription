...
def get_major_scale(key: str) -> list[str]:
    ...
    # 여기서는 단순히 키의 메이저 스케일을 하드코딩하지 않고, chord_to_notes에서 처리합니다.
    return MAJOR_SCALE
    ...
    for chord_str in chord_progression:
        ...
        if instrument_info["name"] == "Piano":
            ...
            for i, note_number in enumerate(notes):
                ...
                # Ensure note_end does not exceed the fixed length
                note_end = min(note_end, start_time + chord_duration)
                note = pretty_midi.Note(
                    velocity=velocity,
                    pitch=note_number,
                    start=note_start,
            ...
            for hit in pattern:
                velocity = random.randint(80, 120)
                note_start = hit["time"]
                note_end = note_start + 0.1  # Short duration for percussive hit
                # Ensure note_end does not exceed fixed length
                note_end = min(note_end, start_time + chord_duration)
                note = pretty_midi.Note(
                    velocity=velocity,
                    pitch=hit["note"],
...
def ensure_fixed_length(wav_file: str, fixed_length: float):
    ...
    If longer, trim it to fixed_length.
    If shorter, pad with silence.
    """
    audio = AudioSegment.from_wav(wav_file)
    current_length = len(audio) / 1000.0  # milliseconds to seconds

    # print(f"Original length of {wav_file}: {current_length:.2f} seconds")

    if current_length > fixed_length:
        # Trim to fixed_length
        audio = audio[: int(fixed_length * 1000)]
        # print(f"Trimmed {wav_file} to {fixed_length} seconds")
    elif current_length < fixed_length:
        # Pad with silence
        silence_duration = (fixed_length - current_length) * 1000
        silence = AudioSegment.silent(duration=silence_duration)
        audio += silence
        # print(f"Padded {wav_file} with {silence_duration/1000.0} seconds of silence")

    # Recalculate to confirm
    new_length = len(audio) / 1000.0
    # print(f"New length of {wav_file}: {new_length:.2f} seconds")
    ...
    for sample_idx in trange(n_samples):
        ...
        for instrument_info in instruments:
            midi_filename = os.path.join(
                MIDI_DIR, str(sample_idx), f"{sample_idx}_{instrument_info['name']}.mid"
            )
            os.makedirs(os.path.dirname(midi_filename), exist_ok=True)

            wav_filename = os.path.join(
                WAV_DIR, str(sample_idx), f"{sample_idx}_{instrument_info['name']}.wav"
            )
        ...
        # Merge WAV files
        merged_wav_filename = os.path.join(MERGED_DIR, f"{sample_idx}_merged.wav")
        total_duration = chord_length * chord_duration  # 예: 4 * 2.0 = 8.0 초
        merge_wav_files(
